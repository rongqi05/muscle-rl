...
**Phase 1 已完成**：官方行走复现、肌力参数化、三层动力学闭环审计、上下肢肌力扫描（配对种子）。
当时的记录口径为 175 步 / 3.50 s（终止步未被记录）。

**Phase 1 修复 + 复验已完成**（详见 [`reports/phase2_acceptance_report.md`](../reports/phase2_acceptance_report.md)）：

| 项 | 结果 |
|---|---|
| 策略一致性（SB3 vs 纯 torch） | 126 个观测上 `max|Δa| = 5.96e-08 ≤ 1e-5`，7/7 通过 |
| 记录口径 | 176 步 / **3.52 s**（终止步入账）；运行时间取自实际仿真时间差 |
| 终止语义 | 研究环境统一：`terminated`=物理跌倒/数值异常，`truncated`=规定时长/本地上限，三分且有明确标志 |
| 根节点世界角速度 | 改用 `mj_jacBody(qpos) @ qvel`；旧索引写法角速度偏差实测 0.154–1.224 rad/s |
| 力分解 | `qfrc_constraint` 按接触/限位/equality/摩擦拆开，残差 2.3e-13 |
| 短时基线 | 0.9582 ± 0.0131 m/s，5/5 走完（与旧记录差异已定量解释为口径变化） |
| 肌力扫描 | 下肢 0.5 → 基本全倒（1.80 s）；上肢 0.25 → 仍走完（−3.6%） |
| 长时评估（20 s） | ⚠️ **全部跌倒**（4.46–4.90 s，侧向失稳），归因于策略自身稳定性而非参考循环 |

**下一步路线**（与原先一致，但优先级受长时结果影响）：

1. **先解决长时稳定性**（新增，最关键）。官方策略 4.6 s 就侧向失稳，比计划的训练回合短得多。
   可选：(a) 用更短回合（≤ 2 个周期）先做损伤适应；(b) 域随机化 + 课程学习把正常肌力的长时
   稳定性练上来再做损伤；(c) 把侧向漂移显式作为终止/惩罚项。三种都会改变与官方基线的可比性，
   必须记为**命名实验**。
2. **域随机化重训**：把上下肢倍率作为训练期随机化维度，才能区分"适应后策略"与"固定策略"。
   新增观测/动作一律放进独立配置，不改官方 checkpoint 接口。
   训练必须走 `hemirl/research_env.py: build_research_env()`（训练/评估共用同一语义）。
3. **不依赖参考的平衡任务**：现有奖励是参考轨迹跟踪，与"平衡能力"不等价；
   研究环境的 `reward_mode='split'` 已把 `imitation` / `energy` / `survival_physical`（不依赖参考）
   / `official_healthy`（依赖参考）分开返回，可直接作为起点。
4. **被动拐杖**：刚体 + 接触 + `eq_active` 侧别开关，保持不变 `nu`，可继续用官方权重评估
   "支撑改变后的动力学"。
5. **主动拐杖**：需要额外的 actuator（会改变 `nu`），必须重新训练。
...
- [`reports/phase2_acceptance_report.md`](../reports/phase2_acceptance_report.md) — **第一阶段修复验收报告**（逐项状态、口径变更、长时失败归因、下一阶段入口与阻塞）
- [`reports/phase1_report.md`](../reports/phase1_report.md) — 阶段一历史记录（数字使用修复前口径）
